...
class RMTSelfAttention(nn.Module):
    ...
    def __init__(self, Dk: int, Dv: int, n_head: int, dropout: float = 0.0,
        ...
        super().__init__()
        self.Dk, self.Dv, self.H = Dk, Dv, n_head

        self.read_q = nn.Linear(Dk, n_head, bias=False)
        self.read_k = nn.Linear(Dk, n_head, bias=False)
        self.read_v = nn.Linear(Dk, n_head, bias=False)

        self.write_o = nn.Linear(n_head, Dk, bias=False)
    ...
    def _retrieve(self, X, reader: nn.Linear):
        # X: (B,T,Dk,Dv) -> (B,H,T,Dv)
        B, T, Dk, Dv = X.shape
        Y = X.transpose(0, 1, 3, 2)            # (B,T,Dv,Dk)
        Y = Y.reshape(B * T * Dv, Dk)          # (B*T*Dv, Dk)
        Y = reader(Y)                          # (B*T*Dv, H)
        Y = Y.reshape(B, T, Dv, self.H).transpose(0, 3, 1, 2)  # (B,H,T,Dv)
        return Y

    def __call__(self, X, causal_mask, cache=None):
        Q = self._retrieve(X, self.read_q)
        K = self._retrieve(X, self.read_k)
        V = self._retrieve(X, self.read_v)

        # Apply RoPE in (B,T,H,Dv) view
        Q = Q.transpose(0, 2, 1, 3)  # (B,T,H,Dv)
        K = K.transpose(0, 2, 1, 3)
        Q = self.rope(Q)
        K = self.rope(K)
        Q = Q.transpose(0, 2, 1, 3)  # (B,H,T,Dv)
        K = K.transpose(0, 2, 1, 3)
        ...
        Y = fast.scaled_dot_product_attention(Q, K, V, scale=scale, mask=causal)  # (B,H,T,Dv)
        Y = self.attn_dropout(Y)

        # Write back to matrix
        Y_t = Y.transpose(0, 2, 3, 1)     # (B,T,Dv,H)
...
class RMTMLP(nn.Module):
    ...
    def __init__(self, Dk: int, Dv: int, n_head: int, dropout: float = 0.0):
        ...
        self.read_f = nn.Linear(Dk, n_head, bias=False)   # Dk -> H
        self.write_f = nn.Linear(n_head, Dk, bias=False)  # H -> Dk
    ...
    def _retrieve(self, X):
        ...
        Y = X.transpose(0, 1, 3, 2)            # (B,T,Dv,Dk)
        Y = Y.reshape(B * T * Dv, Dk)          # (B*T*Dv, Dk)
        Y = self.read_f(Y)                     # (B*T*Dv, H)
        Y = Y.reshape(B, T, Dv, self.H).transpose(0, 3, 1, 2)  # (B,H,T,Dv)
        return Y

    def __call__(self, X):
        Z = self._retrieve(X)   # (B,H,T,Dv)
        B, H, T, Dv = Z.shape
        Z2 = Z.reshape(B * H * T, Dv)
        a = self.fc_gate(Z2)
        b = self.fc_val(Z2)
        y = a * mx.sigmoid(a) * b
        y = self.fc_out(y)
        ...
        Yt = Y.transpose(0, 2, 3, 1)          # (B,T,Dv,H)
...
class RMTBlock(nn.Module):
    def __init__(self, Dk, Dv, n_head, dropout=0.0, rope_base=10000.0, rope_scale=1.0):
        super().__init__()
        self.ln1 = MatrixRMSNorm(Dv)
        self.attn = RMTSelfAttention(Dk, Dv, n_head, dropout, rope_base, rope_scale)
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        Attributes:
            ...
            mlp (MLP): Multi-Layer Perceptron sub-block with SwiGLU activation.
        ...
        Notes:
            ...
            - The `mlp` layer represents the Multi-Layer Perceptron sub-block with SwiGLU activation.
        """
        super().__init__()
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        if config.use_rmt:
            ...
            self.blocks = [
                RMTBlock(
                    Dk=config.Dk, Dv=config.Dv, n_head=config.n_head,
